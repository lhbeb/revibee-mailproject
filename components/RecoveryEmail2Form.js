...
export default function RecoveryEmail2Form({ activeWebsite = 'casoodo' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', content: `Recovery email 2 sent successfully via ${activeWebsite.toUpperCase()}! 💚` });
        setRawData('');
    ...
    <div className="w-full">
      <div className="text-center mb-6">
        <p className="text-gray-600 font-medium">
          Recovery — Friendly Reminder — Sending as <span className="font-bold text-[#003099] uppercase">{activeWebsite}</span>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {isLoading ? (
            ...
            `💚 Send Recovery Email 2 (${activeWebsite.toUpperCase()})`
          )}
        </button>
      </form>
