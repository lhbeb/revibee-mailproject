...
export default function OrderConfirmationForm({ activeWebsite = 'casoodo' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', content: `Order confirmation sent successfully via ${activeWebsite.toUpperCase()}! 🎉` });
        setRawData('');
      } else {
        setMessage({
          type: 'error',
    ...
    <div className="w-full">
      <div className="text-center mb-6">
        <p className="text-gray-600 font-medium">
          Order Confirmation Dashboard — Sending as <span className="font-bold text-[#003099] uppercase">{activeWebsite}</span>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          {isLoading ? (
            ...
            `📧 Send Order Confirmation (${activeWebsite.toUpperCase()})`
          )}
        </button>
      </form>
