...
export default function Home() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#003099] flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-14 w-14 border-4 border-[#FFFBB6] border-t-transparent mx-auto" />
  ...
  if (!isAuthenticated) {
    return (
      <main className="min-h-screen bg-[#003099] flex items-center justify-center p-4">
  ...
  return (
    <main className="min-h-screen bg-[#F0F6FF] flex flex-col">

      {/* ── Top Header ── */}
      <header className="bg-[#003099] border-b border-white/10 shrink-0">
        <div className="max-w-[1400px] mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 bg-white/10 px-3 py-1.5 rounded-lg">
              <div className="w-6 h-6 rounded-full bg-[#FFFBB6] flex items-center justify-center text-[#003099] text-xs font-black">
      ...
      <div className="flex-1 max-w-[1400px] mx-auto w-full px-6 py-8 flex flex-col gap-6">
        ...
        <div className="flex gap-0 bg-[#F0F6FF] rounded-2xl shadow-lg border border-slate-200 overflow-hidden" style={{ minHeight: '640px' }}>

          {/* Left Sidebar */}
          <aside className="w-64 shrink-0 bg-[#003099] flex flex-col">
                        ...
                        <li key={item.id}>
                          <button
                            ...
                            className={`w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 group
                              ${isActive
                                ? 'bg-[#FFFBB6] text-[#003099]'
                                : 'text-white/60 hover:text-white hover:bg-white/10'
                              }`}
                          >
                            <span className="text-lg shrink-0">{item.icon}</span>
                            <div className="min-w-0">
                              <p className={`text-sm font-semibold leading-tight truncate ${isActive ? 'text-[#003099]' : ''}`}>{item.label}</p>
                              <p className={`text-[10px] leading-tight mt-0.5 truncate ${isActive ? 'text-[#003099]/60' : 'text-white/30'}`}>{item.sub}</p>
                            </div>
                            {isActive && (
                              <span className="ml-auto shrink-0 w-1.5 h-1.5 rounded-full bg-[#003099]/50" />
                            )}
                          </button>
                        </li>
                      );
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="px-8 py-5 border-b border-slate-100 flex items-center gap-4 shrink-0">
              <div className="w-10 h-10 rounded-xl bg-[#F0F6FF] flex items-center justify-center text-xl shrink-0">
                {activeItem.icon}
              </div>
              <div>
                <h2 className="text-lg font-bold text-[#003099] leading-tight">{activeItem.label}</h2>
        ...
        <div className="bg-[#F0F6FF] rounded-2xl shadow-lg border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-[#F0F6FF] flex items-center justify-center text-lg">📋</div>
            <div>
              <h2 className="text-base font-bold text-[#003099]">Sent Emails History</h2>
